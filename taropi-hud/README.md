...
基础区域包含三条逻辑行：会话目录、Git、模型、上下文和 Token 用量汇总，以及 `idle agent:`、`working agent:` 两组本机交互会话目录。目录会先按完整路径在每组内去重，再使用 Fish 风格缩写；同一路径可同时出现在两组。空组显示 `—`，终端较窄时由 TUI 自动折行显示全部目录。

会话加载本版 HUD 后会在当前用户的运行时目录内上报最小状态（进程、会话标识、目录、状态、心跳），不会读取会话正文。普通输入框和确认/问答等待属于 idle，模型、工具、重试和压缩执行属于 working。异常退出记录会由进程存活检测和 15 秒心跳过期清理；其他终端状态变化最多约 1 秒后显示。
